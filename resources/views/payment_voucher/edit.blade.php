        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-10 offset-lg-1">
                    <form action="{{ route('payment-vouchers.update', $paymentVoucher->id) }}"
                        ...
                        <div class="card card-info">
                            <div class="card-header">
                                <h3 class="card-title mb-0">
                                    ...
                                    <span class="font-weight-normal ml-1">{{ $paymentVoucher->uid }}</span>
                                </h3>
                                <div class="card-tools">
                                    <a href="{{ route('payment-vouchers.show', encrypt($paymentVoucher->id)) }}"
                            ...
                            <div class="card-body">
                                <p class="text-muted small mb-3">
                                    Money paid from <strong>Cash/Bank (Cr)</strong> to an
                                    <strong>expense / payable account (Dr)</strong>.
                                </p>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input type="number" class="form-control" id="amount" name="amount"
                                                   ...
                                                   value="{{ old('amount', $paymentVoucher->amount) }}" required>
                                            @error('amount')
                                                <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="debit_account_id">
                                                Debit Account (Dr)
                                                <small class="text-muted">Expense / Payable</small>
                                            ...
                                            <select class="form-control select2" name="debit_account_id"
                                                ...
                                                @foreach ($debitAccounts as $row)
                                                    <option value="{{ $row->id }}"
                                                        {{ (string) old('debit_account_id', $paymentVoucher->debit_account_id) === (string) $row->id ? 'selected' : '' }}>
                                                        {{ $row->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" name="credit_account_id"
                                                ...
                                                @foreach ($creditAccounts as $row)
                                                    <option value="{{ $row->id }}"
                                                        {{ (string) old('credit_account_id', $paymentVoucher->credit_account_id) === (string) $row->id ? 'selected' : '' }}>
                                                        {{ $row->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="payee_name" name="payee_name"
                                                   ...
                                                   value="{{ old('payee_name', $paymentVoucher->payee_name) }}" required>
                                            @error('payee_name')
                                                <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
                                ...
                                <div class="form-group mb-0">
                                    ...
                                    <textarea class="form-control" name="narration" id="narration" rows="3"
                                              placeholder="Optional description">{{ old('narration', $paymentVoucher->narration) }}</textarea>
                                    @error('narration')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                            ...
                            <div class="card-footer text-right">
                                <a href="{{ route('payment-vouchers.index') }}" class="btn btn-default">Cancel</a>
