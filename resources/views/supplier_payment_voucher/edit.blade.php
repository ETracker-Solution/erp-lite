@extends('layouts.app')
@section('title', 'Supplier Payment Voucher Edit')
@section('content')
    @php
        $links = [
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row" id="vue_app">
                ...
                <div class="col-md-12">
                    <form action="{{ route('supplier-vouchers.update', $voucher->id) }}" method="POST" class=""
                          enctype="multipart/form-data">
                        @method('put')
                        @csrf
                        <div class="card">
                            <div class="card-header bg-info">
                                <h4 class="card-title">Supplier Payment Voucher (SPV) Edit - {{ $uid }}</h4>
        ...
        $(document).ready(function () {
            var vue = new Vue({
                ...
                data: {
                    ...
                    reference_no: '',
                    suppliers: [],
                    selected_items: {!! json_encode($supplierVouchers->map(function($sv) {
                        return [
                            'credit_account_id' => $sv->credit_account_id,
                            'credit_account_name' => $sv->creditAccount->name ?? '',
                            'supplier_id' => $sv->supplier_id,
                            'supplier_name' => $sv->supplier->name ?? '',
                            'amount' => $sv->amount,
                            'payee_name' => $sv->payee_name,
                            'reference_no' => $sv->reference_no,
