@extends('layouts.app')
@section('title', 'Customer Due Receive Voucher Edit')
@section('content')
    @php
        $links = [
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row" id="vue_app">
                ...
                <div class="col-md-12">
                    <form action="{{ route('customer-receive-vouchers.update', $voucher->id) }}" method="POST" class=""
                          enctype="multipart/form-data">
                        @method('put')
                        @csrf
                        <div class="card">
                            <div class="card-header bg-info">
                                <h4 class="card-title">Customer Due Receive Voucher (CRV) Edit - {{ $uid }}</h4>
        ...
        $(document).ready(function () {
            var vue = new Vue({
                ...
                data: {
                    ...
                    due: 0,
                    invoices: [],
                    selected_items: {!! json_encode($customerReceiveVouchers->map(function($sv) {
                        return [
                            'debit_account_id' => $sv->debit_account_id,
                            'debit_account_name' => $sv->debitAccount->name ?? '',
                            'customer_id' => $sv->customer_id,
                            'customer_name' => $sv->customer->name,
                            'sale_id' => $sv->sale_id,
                            'invoice_number' => $sv->sale->invoice_number,
                            'amount' => $sv->amount,
                            'settle_discount' => $sv->settle_discount
                        ];
                    })) !!},
                    pageLoading: false,
                    uid: '{{ $uid }}',
