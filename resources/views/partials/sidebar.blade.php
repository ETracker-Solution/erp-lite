<aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <a href="{{config('sidebar-menus.logo.url')}}" class="brand-link">
        <img src="{{ asset('upload').'/'.getSettingValue('company_logo') }}" alt="{{ config('sidebar-menus.logo.alt') }}"
        ...
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                @foreach(config('sidebar-menus.menus') as $mainMenu)
                    @canany($mainMenu['permissions'] ?? getAllPermissions())
                                            ...
                                            @if($level2Menu['has_child'])
                                                <ul class="nav nav-treeview">
                                                    @foreach($level2Menu['child'] as $level3Menu)
                                                        <li class="nav-item">
                                                            <a href="{{ $level3Menu['url']}}"
                                                               class="nav-link {{ $level3Menu['active_condition'] ? 'active' : ''  }}">
